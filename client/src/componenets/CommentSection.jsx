...
export default function CommentSection({ postId }) {
    ...
    return (
        <div className='max-w-2xl mx-auto w-full p-3'>
            {currentUser ? (
                <div className='flex items-center gap-1 my-5 text-gray-300 text-m'>
                    <p>Signed in as:</p>
                    <img className='h-5 w-5 object-cover rounded-full' src={currentUser.profilePicture} alt='' />
                    <Link to={'/dashboard?tab=profile'} className='text-m text-cyan-300 hover:underline'>
                        @ {currentUser.username}
                    </Link>
                </div>
            ) : (
                <div className='text-m text-teal-300 my-5'>You must be signed in to comment.
                    <Link className='text-blue-300 hover:underline flex gap-1' to={'/sign-in'}>
                        Sign in
                    </Link>
                </div>
            )}
